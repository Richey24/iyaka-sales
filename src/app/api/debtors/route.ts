...
export async function GET(request: NextRequest) {
    try {
        ...
        const debtors = await Debtor.find({
            ...
            .limit(Number(limit))
            .sort({ createdAt: -1 });
        const total = await Debtor.countDocuments({ companyId: decoded.companyId });
        return NextResponse.json({ debtors, total, totalPages: Math.ceil(total / Number(limit)) }, { status: 200 });
    } catch (error) {
        return NextResponse.json({ message: "Internal server error", error }, { status: 500 });
    }
}
